...
@app.route('/dict.html', methods=['GET'])
def get_dict():
    """
    百科
    """
...
def pets_classify():
    ...
    res = {
        ...
        'data': {
            ...
            'msg': '<br><strong style="font-size: 48px;"'
                   '>{}</strong><br><br><span style="font-size: 24px;"'
                   '>概率<strong>{}</strong></span>'
                .format(pet_class, pet_cls_prob),
        }
    }
    # 返回json数据
    return jsonify(res)
...
if __name__ == '__main__':
    app.run(use_reloader=False, host='0.0.0.0', port=settings.WEB_PORT, debug=Flask)
